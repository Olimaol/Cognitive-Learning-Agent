...
simID=sys.argv[1]
sigV1=["normal", "wide", "small"][int(sys.argv[2])]
...
def choose(orientation,mode):
	if orientation==-1:
		...
		if mode==1:
			if sum(np.array(params['possibleOr'])==orientation)==1:
				a=SNr.baseline
				b=PFC.baseline
				i=int((orientation-10)/5)
				#a[:]=changed['baseline_SNr']+0.12
				#a[i]=changed['baseline_SNr']-1.0
				b[:]=-1000
				b[i]=1000
				SNr.baseline=a
				PFC.baseline=b
...
if __name__ == '__main__':
	...
	if(int(simID)%parallelAnz!=0):
		...
		compNr=parallelAnz
	ann.compile(directory="annarchy_sim"+str(compNr))
	

	#####################################################
	##############  PREPARE SIMULATION   ################
	#####################################################
	MAXSIMULATIONSZEIT=3600*3
	maxTrainingTrials=1
	trainStim='t1'
	testStim='t2'
	tMaxtrainStim=200
	...
	### LOAD STIMULI  ###
	Input = np.zeros((numDifStims, 41, 31, 1, 16))
	Input_img = np.zeros((numDifStims, 41, 31))
	Input_lineCoords = []
	Input_phi = []
	for stim_nbr in np.arange(numDifStims):
		Input[stim_nbr]=np.load("new_stims_"+sigV1+"/input"+str(stim_nbr)+".npy")
		Input_img[stim_nbr]=np.load("new_stims_"+sigV1+"/img"+str(stim_nbr)+".npy")
		Input_lineCoords.append(np.load("new_stims_"+sigV1+"/lineCoords"+str(stim_nbr)+".npy"))
		Input_phi.append(np.load("new_stims_"+sigV1+"/phi"+str(stim_nbr)+".npy"))
	distList=[]

    
	folder='2020_09_18_WirkungPFC_'+sigV1+'/'
	try:
	...
	while training==1 and ((training==1 and trial<maxTrainingTrials) or (training==0)) and (BlockTrial<(trainStim_Anz+testStim_Anz)) and ((timer()-SIMULATIONSSTART)<MAXSIMULATIONSZEIT):
		...
		IList=np.array(Input_lineCoords[I])
		...
		###  set selected gain  ###
		ann.simulate(20)
		choose(orientations[(int(simID)-1)//10],1) 
		...
		V1.baseline = 0
		choose(orientations[(int(simID)-1)//10],0) 
	...
	np.save('data/'+folder+'/IList'+str(simID)+'.npy',IList)

...
def gridStim(nr, phi=[50,50,50,50,50,50,50,50,55]):
	img = np.ones((params['x_max'],params['y_max']), np.uint8)*(-1)
...
def createStimKerzelTest(nr):
	return gridStim(nr, [90,90,90,105,105,105,120,120,50])
...
v1Act= np.zeros((params['V1_shape'][0],params['V1_shape'][1],params['V1_shape'][2],params['V1_shape'][3]))
sigV1 = ['','normal','wide','small'][int(sys.argv[1])]
sigV1_Value = {'normal': 35, 'wide': 47.43, 'small': 22.14, '': 35}[sigV1]
print(sigV1_Value)
for stimNr in range(450):
	...
	elif stimNr>=400 and stimNr<=449:
		[img,centerList,phi]=createStimKerzelTest(stimNr)	
	
	for filterIdx in range(len(filters)):
		v1Act[:,:,0,filterIdx]=gauss(img,filters[filterIdx],sigV1_Value)#V4 Orientation-Tuning bandwidth ca 52 (median laut Visual Properties of Neurons in Area V4 of the Macaque: Sensitivity to Stimulus Form)

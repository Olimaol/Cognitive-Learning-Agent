...
MaxPooling = ann.Synapse(
    ...
    description = "post-synaptic neurons use the maximum psp of all incoming synapses"
)
...
if __name__ == '__main__':
	...
	if(int(simID)%parallelAnz!=0):
		...
		compNr=parallelAnz
	ann.compile(directory="annarchy_sim"+str(compNr+[0,5,0][trainStimID]))
	ann.report(filename="report/model_description.tex", standalone=False)
	...
	folder='2020_11_11_testMainExperiment_'+['T1','T1rev'][trainStimID]+'/'
	...
	BlockTrial=0
	numCorrect=0
	while ((training==1 and trial<maxTrainingTrials) or (training==0)) and (BlockTrial<(trainStim_Anz+testStim_Anz)) and ((timer()-SIMULATIONSSTART)<MAXSIMULATIONSZEIT):
		###  trial start time  ###
